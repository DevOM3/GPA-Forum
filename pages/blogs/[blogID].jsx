...
const Blog = () => {
  ...
  const [blogSummarization, setBlogSummarization] = useState("");
  const [blogData, setBlogData] = useState({});
  const [userData, setUserData] = useState({});
  const [openBlogCopy, setOpenBlogCopy] = React.useState(false);
  const [openSummarization, setOpenSummarization] = React.useState(false);
  const theme = useTheme();
  const fullScreen = useMediaQuery(theme.breakpoints.down("sm"));

  const handleClickOpenSummarization = () => {
    setOpenSummarization(true);
  };

  const handleCloseSummarization = () => {
    setOpenSummarization(false);
  };
  ...
  const summarize = async () => {
    handleClickOpenSummarization();
    if (!blogSummarization) {
      const response = await axios.post(
        `https://textsummerizer.herokuapp.com/?blog=${blogData?.text}`
      );
      setBlogSummarization(response.data.summary);
    }
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        <DialogContent>
          {blogSummarization ? (
            <DialogContentText>{blogSummarization}</DialogContentText>
          ) : (
            <CircularProgress />
          )}
        </DialogContent>
        <DialogActions>
          <Button autoFocus onClick={handleCloseSummarization} color="primary">
            Close
          </Button>
        </DialogActions>
      </Dialog>
      ...
      <div className={blogsStyles.blog}>
        ...
        <div className={blogsStyles.buttons}>
          ...
          <button className={blogsStyles.button} onClick={summarize}>
            Summarize
          </button>
